...
static uint8_t
map_percent_sw102(uint8_t percent)
{
	/*
	 * Measured on Display
	 * -----------------------
...
battery_process(void)
{
	if (!first_reading_done) {
		...
		if (target_current == 0 &&
		    (system_ms() - motor_disabled_at_ms) > BATTERY_NO_LOAD_DELAY_MS) {
			battery_percent = compute_battery_percent();
		}
	}
}
...
uint8_t
battery_get_percent(void)
{
	return battery_percent;
}

uint8_t
battery_get_mapped_percent(void)
{
#if (BATTERY_PERCENT_MAP == BATTERY_PERCENT_MAP_SW102)
