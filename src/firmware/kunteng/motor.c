...
#include "kunteng/stm8s/stm8s_flash.h"
...
/* hard current limits */
#define MAX_BATTERY_CURRENT_AMPS_X10			100
#define MAX_MOTOR_PHASE_CURRENT_AMPS_X10		200
 ...
 * You can try to tune with the wheel on the air, full throttle
 * and look at batttery current: adjust for lower battery current
 */
#define MOTOR_ROTOR_OFFSET_ANGLE			208
...
#define MOTOR_ROTOR_ANGLE_90				(u8)(63  + MOTOR_ROTOR_OFFSET_ANGLE)
#define MOTOR_ROTOR_ANGLE_150				(u8)(106 + MOTOR_ROTOR_OFFSET_ANGLE)
#define MOTOR_ROTOR_ANGLE_210				(u8)(148 + MOTOR_ROTOR_OFFSET_ANGLE)
#define MOTOR_ROTOR_ANGLE_270				(u8)(191 + MOTOR_ROTOR_OFFSET_ANGLE)
#define MOTOR_ROTOR_ANGLE_330				(u8)(233 + MOTOR_ROTOR_OFFSET_ANGLE)
#define MOTOR_ROTOR_ANGLE_30				(u8)(20  + MOTOR_ROTOR_OFFSET_ANGLE)
...
/* set how often the current controller runs in the isr */
...
volatile bool oc_cooldown = false;
...
static void
flash_opt2_afr5(void)
{
	/* verify if PWM N channels are active on option bytes, if not, enable */
	const uint8_t wanted_optval = 0x20;

	if (OPT->OPT2 != wanted_optval) {
		/* unlock data memory */
		...
		OPT->OPT2 = wanted_optval;
		OPT->NOPT2 = (uint8_t)(~wanted_optval);
...
static uint8_t
asin_table(uint8_t inverted_angle_x128)
{
	uint8_t index = 0;
...
motor_pre_init(void)
{
	/* OverCurrent TLI(Top Level Interrupt) */
	SET_PIN_INPUT_IT(OC_IOT);

	/* hall sensors */
	SET_PIN_INPUT(HALL_IOT);

	/* not touching PWM pins here(nor anywhere else), as i might have fried one controller doing that */

	/* ADC inputs - missing X4 i think */
	SET_PIN_INPUT(ADC_B_IOT);	/* throttle, current phase b, raw shunt current */
	SET_PIN_INPUT(ADC_E_IOT);	/* filtered shunt current, battery voltage */

#if 1
	/* brake w/no irq */
	SET_PIN_INPUT(BRAKE_IOT);
#else
	/* brake w/exti sensitivity for irq triggering on falling only */
	SET_PIN_INPUT_IT(BRAKE_IOT);
	EXTI->CR1 &= (u8)~EXTI_CR1_PAIS;
	EXTI->CR1 |= (u8)EXTI_SENSITIVITY_FALL_ONLY;
#endif

	/* PAS */
/* XXX	SET_PIN_INPUT_PULLUP(PAS_IOT);*/

	/* speed sensor */
	SET_PIN_INPUT_PULLUP(SPEED_IOT);

	/* X8 == to be used as button input */
	SET_PIN_INPUT(X8_IOT);

	/* pins used as outputs for dbg(for viewing w/logic analyzer(+pulseview)) */
#ifdef	DBG_P0_IOT
	SET_PIN_LOW(DBG_P0_IOT);
	SET_PIN_OUTPUT(DBG_P0_IOT);
#endif
#ifdef	DBG_P1_IOT
	SET_PIN_LOW(DBG_P1_IOT);
	SET_PIN_OUTPUT(DBG_P1_IOT);
#endif
#ifdef	DBG_P2_IOT
	SET_PIN_LOW(DBG_P2_IOT);
	SET_PIN_OUTPUT(DBG_P2_IOT);
#endif
#ifdef	DBG_P3_IOT
	SET_PIN_LOW(DBG_P3_IOT);
	SET_PIN_OUTPUT(DBG_P3_IOT);
#endif
#ifdef	DBG_P4_IOT
	SET_PIN_LOW(DBG_P4_IOT);
	SET_PIN_OUTPUT(DBG_P4_IOT);
#endif
#ifdef	DBG_P5_IOT
	SET_PIN_LOW(DBG_P5_IOT);
	SET_PIN_OUTPUT(DBG_P5_IOT);
#endif
#ifdef	DBG_P6_IOT
	SET_PIN_LOW(DBG_P6_IOT);
	SET_PIN_OUTPUT(DBG_P6_IOT);
#endif
#ifdef	DBG_P7_IOT
	SET_PIN_LOW(DBG_P7_IOT);
	SET_PIN_OUTPUT(DBG_P7_IOT);
#endif

	/* unused pins */
	SET_PIN_INPUT_PULLUP(GPIOA, (GPIO_PIN_3 | GPIO_PIN_5 | GPIO_PIN_6));
	SET_PIN_INPUT_PULLUP(GPIOB, GPIO_PIN_3);
	SET_PIN_INPUT_PULLUP(GPIOC, GPIO_PIN_6);
	SET_PIN_INPUT_PULLUP(GPIOD, GPIO_PIN_1);
	SET_PIN_INPUT_PULLUP(GPIOE, GPIO_PIN_5);
	SET_PIN_INPUT_PULLUP(GPIOG, (GPIO_PIN_0 | GPIO_PIN_1));
}

void
motor_init(uint16_t max_current_mA, uint8_t lvc_V, int16_t adc_calib_volt_step_offset)
{
	lvc_x10V = lvc_V * 10;
	...
	adc_low_voltage_limit = (u16)(((u32)lvc_V * adc_steps_per_volt_x512) / 512);

	flash_opt2_afr5();
	timer1_init_motor_pwm();
	motor_disable();

	/* forcing to run these so values(/averages) get filled */
	for (u8 i = 0; i < 32; i++) {
		/*adc_trigger_scan_eoc(ADC1_CHANNEL_BATTERY_VOLTAGE);*/
		adc_trigger_all = 1;
		system_delay_ms(2);
		adc_process();
		motor_process();
...
uint8_t
motor_get_target_speed(void)
{
	return target_speed_percent;
}

uint8_t
motor_get_target_current(void)
{
	return target_current_percent;
}

void
motor_set_target_speed(uint8_t percent)
{
...
void
motor_set_target_current(uint8_t percent)
{
...
motor_calibrate_battery_voltage(uint16_t actual_voltage_x100)
{
	...
	if (actual_voltage_x100 != 0) {
		u16 calibrated_adc_steps_volt_x512 = (u16)
		    (((uint32_t)adc_battery_voltage_filtered * 51200u) / actual_voltage_x100);
...
uint16_t
motor_get_battery_lvc_x10(void)
{
	return lvc_x10V;
}

uint16_t
motor_get_battery_current_x10(void)
{
	return (uint16_t)((((uint32_t)adc_battery_current_filtered * 10) * ADC_10BIT_CURRENT_PER_ADC_STEP_X512) >> 9);
}

uint16_t
motor_get_battery_voltage_x10(void)
{
	return (uint16_t)(((uint32_t)adc_battery_voltage_filtered * 5120) / adc_steps_per_volt_x512);
}

void
isr_oc_tli(void) __interrupt(ITC_IRQ_TLI)
{
	volatile bool oc = GET_PIN_INPUT_STATE(OC_IOT) ? false : true;	/* active low */

	if (oc) {
		TIM1->BKR &= (u8)~TIM1_BKR_MOE;	/* disable outputs */
		TIM1->CCER1 &= (u8)~(TIM1_CCER1_CC1E | TIM1_CCER1_CC1NE);	/* OC1 */
		TIM1->CCER1 &= (u8)~(TIM1_CCER1_CC2E | TIM1_CCER1_CC2NE);	/* OC2 */
		TIM1->CCER2 &= (u8)~(TIM1_CCER2_CC3E | TIM1_CCER2_CC3NE);	/* OC3 */
		control_state = CONTROL_STATE_DISABLE;
		do {
			TOGGLE_PIN(DBG_P0_IOT);
			TOGGLE_PIN(DBG_P1_IOT);
			TOGGLE_PIN(DBG_P2_IOT);
			TOGGLE_PIN(DBG_P3_IOT);
			TOGGLE_PIN(DBG_P4_IOT);
			TOGGLE_PIN(DBG_P5_IOT);
			TOGGLE_PIN(DBG_P6_IOT);
			TOGGLE_PIN(DBG_P7_IOT);
			oc = GET_PIN_INPUT_STATE(OC_IOT) ? false : true;
		} while (oc);
		SET_PIN_LOW(DBG_P0_IOT);
		SET_PIN_LOW(DBG_P1_IOT);
		SET_PIN_LOW(DBG_P2_IOT);
...
static const uint8_t hall_angles[8] = {
	0,			/* invalid */
	MOTOR_ROTOR_ANGLE_270,	/* 1 0b001 */
	MOTOR_ROTOR_ANGLE_150,	/* 2 0b010 */
	MOTOR_ROTOR_ANGLE_210,	/* 3 0b011 */
	MOTOR_ROTOR_ANGLE_30,	/* 4 0b100 */
	MOTOR_ROTOR_ANGLE_330,	/* 5 0b101 */
	MOTOR_ROTOR_ANGLE_90,	/* 6 0b110 */
	0			/* invalid */
};
static volatile uint8_t fullrev_done = 0;
 ...
 */
void
isr_timer1_cmp(void) __interrupt(ITC_IRQ_TIM1_CAPCOM)
{

	SET_PIN_HIGH(DBG_P0_IOT);
	/* grab the latest continuous conversion value */
	volatile u8 hi = ADC1->DRH;
	volatile u8 lo = ADC1->DRL;
#if 1
	ADC1->CR1 = ADC1_CR1_ADON; /* disable continuous conversion */
#endif
#if (ADC_CONT_CH == ADC1_CHANNEL_BATTERY_CURRENT)
	adc_battery_current = (u16)(hi << 2) | lo;
/*	adc_battery_current -= adc_battery_zero_current;*/
#define	adc_cont_current	adc_battery_current
#elif (ADC_CONT_CH == ADC1_CHANNEL_ISHUNT_CURRENT)
	adc_ishunt_current = (u16)(hi << 2) | lo;
#define	adc_cont_current	adc_ishunt_current
#endif

	/* will trigger conversion for throttle, and go on continous for current after that */
	adc_trigger_eoc(ADC1_CHANNEL_BATTERY_VOLTAGE);

#if 0
/*	adc_battery_current = adc_update_avg_current();*/
#if (ADC_CONT_CH == ADC1_CHANNEL_BATTERY_CURRENT)
	...
	case CONTROL_STATE_PREPARE:
		if (speed_erps > 0) {
			 ...
			 */
			pwm_duty_cycle = (uint8_t)MAP32(speed_erps, 0, MAX_MOTOR_SPEED_ERPS,
			    PWM_DUTY_CYCLE_MIN, PWM_DUTY_CYCLE_MAX);
		}	
		control_state = CONTROL_STATE_START;
		break;
	case CONTROL_STATE_START:
		SET_PIN_HIGH(DBG_P5_IOT);
		/* enable outputs */
		TIM1->CCER1 |= (uint8_t)(TIM1_CCER1_CC1E | TIM1_CCER1_CC1NE); 	/* OC1 */
		TIM1->CCER1 |= (uint8_t)(TIM1_CCER1_CC2E | TIM1_CCER1_CC2NE);	/* OC2 */
		TIM1->CCER2 |= (uint8_t)(TIM1_CCER2_CC3E | TIM1_CCER2_CC3NE);	/* OC3 */
	...
	/* XXX hall sensors sequence with motor forward rotation: 4, 6, 2, 3, 1, 5 */
	volatile u8 hall_sensors = GET_PINS_INPUT_STATE(HALL_IOT);
	/* make sure we run next code only when there is a change on the hall sensors signal */
	if (hall_sensors != hall_sensors_state_last) {
		hall_sensors_state_last = hall_sensors;

		rotor_absolute_angle = hall_angles[hall_sensors/* & 7 */];
		if (rotor_absolute_angle) {
			if (hall_sensors == 3 && half_erps_flag) {
				half_erps_flag = 0;
				fullrev_done = 1;
			} else
			if (hall_sensors == 4) {
	...
	if (commutation_type == SINEWAVE_INTERPOLATION_60_DEGREES) {
		 ...
		 * TODO: verify if (motor_pwm_cycles_counter_6 << 8) do not overflow
		 */
							/* this operations take 4.4us */
		uint8_t interpolation_angle = (pwm_cycles_counter_6 << 8) / pwm_cycles_counter_total;
	...
	if (control_state == CONTROL_STATE_DISABLE ||
	    is_lvc_triggered ||
	    (pwm_duty_cycle_target == 0) ||
	    (GET_PIN_INPUT_STATE(BRAKE_IOT) == 0)) { /* active low */
		if (pwm_duty_cycle)
			--pwm_duty_cycle;
	} else
#if /* XXX this should be first to be fixed*/0
	if (current_controller_counter > CURRENT_CONTROLLER_CHECK_PERIODS &&
	    (adc_battery_current_ovf ||	/* check if truncated 8bit current reading did overflow */
	    adc_battery_current > adc_battery_ramp_max_current ||	/* compare against ramp controller current limit */
	    adc_phase_current > adc_phase_max_current)) {	/* or hard motor phase current limit */
		if (pwm_duty_cycle)
			--pwm_duty_cycle;
	} else
#endif
	if (speed_controller_counter > SPEED_CONTROLLER_CHECK_PERIODS &&	/* test about every 100ms */
...
#define CALC_PHASE(PHASE_OUTPUT) 							\
	...
	/* phase B */
	TIM1->CCR2H = phase_b_voltage_msb;
	TIM1->CCR2L = phase_b_voltage_lsb;
	/* phase C */
	TIM1->CCR1H = phase_c_voltage_msb;
	TIM1->CCR1L = phase_c_voltage_lsb;
	/* phase A */
	TIM1->CCR3H = phase_a_voltage_msb;
	TIM1->CCR3L = phase_a_voltage_lsb;
